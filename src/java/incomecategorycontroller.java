...
public class incomecategorycontroller extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            out.println("<html>");
            out.println("<head>");
            out.println("<title>Servlet incomecategorycontroller</title>");
            out.println("</head>");
            out.println("<body>");
            ...
            String catdet = request.getParameter("inccatdetails");
            String catname = request.getParameter("inccatname");
            IncomeCategoryDao id = new IncomeCategoryDao();
            IncomeCategoryBean ib = new IncomeCategoryBean();
            ib.setInc_catdetails(catdet);
            ib.setInc_catname(catname);
            ib.setUserid(userid);

            int x = id.addIncomeCategory(ib);
            if (x > 0) {
               response.sendRedirect("pisIncomeCategory");
